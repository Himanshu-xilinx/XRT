...
namespace xcldev {
  ...
  public:
    ...
    int getDDRBanks( std::vector<mem_bank_t>& aBanks )
    {
        int nfound = 0;
        aBanks.clear();
        std::string base = "/sys/devices/platform/amba/amba:zyxclmm_drm";
        std::string icap;
        struct stat sb;
        ...
        for( int i = 0; i < map->m_count; i++ ) {
            if( map->m_mem_data[i].m_used && map->m_mem_data[i].m_type != MEM_STREAMING ) {
                ...
                printf("HIMANSHU %s:%d mbase_address:%x size:%x index:%d\n",__func__,__LINE__,map->m_mem_data[i].m_base_address,map->m_mem_data[i].m_size*1024,i);
            }
        }
